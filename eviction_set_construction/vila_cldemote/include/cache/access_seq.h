...
static inline void d_access_array_bwd(u8 **addrs, size_t size) {
    size_t i;
    for (i = size; i > 0; i--) {
        _maccess(addrs[i - 1]);
	cldemote(addrs[i - 1]);
...
static __always_inline void d_prime_cands_daniel(u8 **cands, size_t cnt,
                                               size_t repeat, size_t stride,
                                               size_t block) {
    block = _min(block, cnt);
    for (size_t s = 0; s < cnt; s += stride) {
        for (size_t c = 0; c < repeat; c++) {
            if (cnt >= block + s) {
                d_access_array_bwd(&cands[s], block);
            } else {
                u32 rem = cnt - s;
                d_access_array_bwd(&cands[s], rem);
                d_access_array_bwd(cands, block - rem);
            }
        }
    }
}
