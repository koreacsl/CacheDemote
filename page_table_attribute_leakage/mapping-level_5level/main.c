...
int main(int argc, char* argv[]) {
    ...
    for(size_t i = 1; i < 512; i++) {
        size_t current = start + i * 256ull * 1024ull * 1024ull * 1024ull * 1024ull;
        printf("Checking %p...\n", current);
        ptedit_entry_t entry = ptedit_resolve(current, 0);
	printf("Pbit %d\n", ptedit_cast(entry.pte, ptedit_pgd_t).present);
        ...
        current = start + (i + 1) * 256ull * 1024ull * 1024ull * 1024ull * 1024ull;
        printf("Checking next %p...\n", current);
        entry = ptedit_resolve(current, 0);
        ptedit_print_entry_t(entry);
        if(!ptedit_cast(entry.pte, ptedit_pgd_t).present && first) {
            start = start + i * 256ull * 1024ull * 1024ull * 1024ull * 1024ull;
            break;
        }
    }
